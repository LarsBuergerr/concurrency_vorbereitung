    ...
    for i in 0..10 {
        ...
        let handle = thread::spawn(move || {
            for _ in 0..100 {
                ...
                let res = FactorizerService::service(&service_clone, r_num);
                FactorizerService::print_result(i, r_num, res);
            }
        });
...
impl FactorizerService {
    ...
    fn service(service: &Arc<Mutex<FactorizerService>>, number: u64) -> [u64;100] {
        {
            let mut service = service.lock().unwrap();
            if number == service.LAST_NUMBER {
                println!("cache hit");
                return service.LAST_FACTORS;
            }
        }
        println!("cache miss");
    ...
    fn print_result(thread_id: u64, number: u64, result: [u64;100]) {
